    ...
    public class ProductService : BaseService, IProductService
    {
        ...
        public async Task<ResponseObject<ProductDTO>> UpdateImageProduct(Request_UpdateImageProduct request)
        {
            ...
            try
            {
                ...
                if (!HandleCheckImage.IsImage(request.ImageProduct))
                {
            ...
            productImg.LinkImg = await HandleUploadImage.UpdateFile(productImg.LinkImg, request.ImageProduct);
            _context.productsImg.Update(productImg);
            var product = productImg.Product;
            ...
            await _context.SaveChangesAsync();
            return _responObjectProduct.ResponseSuccess("Thay đổi ảnh sản phẩm thành công", productDTO);
        }
        ...
        public async Task<PageResult<ProductDTO>> GetAllProducts(int pageSize = 10, int pageNumber = 1)
        {
            var productQuery = _context.products
                .Where(product => product.StatusId == 1 && product.IsActive.Value)
                .OrderByDescending(x => x.BatDauBan)
                .Include(x => x.DauChu)
                .Include(x => x.PhieuXemNhas)
                .Include(x => x.ProductImgs)
            ...
            var lstProductDTO = productQuery.Select(x => new ProductDTO()
            {
                Id = x.Id,
                HostName = x.HostName,
                HostPhoneNumber = x.HostPhoneNumber,
                Build = x.Build,
                CertificateOfLand1 = x.CertificateOfLand1,
                CertificateOfLand2 = x.CertificateOfLand2,
                Address = x.Address,
                StatusId = x.StatusId,
                DauChuId = x.DauChuId,
                GiaBan = x.GiaBan,
            ...
            var data = lstProductDTO.Skip(pageSize * (pageNumber - 1)).Take(pageSize).ToList();

            foreach (var product in data)
            {
                product.ProductImgDTOs = _context.productsImg
                                        .Where(x => x.ProductId == product.Id)
                                        .Select(productImg => _productImgConverter.EntityToDTO(productImg));
            }

            var result = Pagination.GetPagedData(data.AsQueryable(), pageSize, pageNumber);
            return result;
        }
